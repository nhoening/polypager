...
*/
//$chroot_dir = '../../user/Image/';  // Note: You may also want to add this path to the
...
$header['force_disa']   = TRUE;  // Force HTML headers to be suppressed.
...
*/
$convert_magick = FALSE;  // Use Image Magick, if available, to convert images.
...
* Miscellaneous
*/
$qdig_lang['nav_cntr_txt']  = 'of'; // Counter ("x of n")
$qdig_lang['Forbidden']     = 'Forbidden';
$qdig_lang['diag_messages'] = 'Diagnostic Messages';
...
$request_vars = array_merge($get_vars, $post_vars, $cookie_vars);
// Protect all global variables from register_globals.
if (ini_get('register_globals')) {
	foreach($request_vars as $k=>$v) {
		if (preg_match('/^(GLOBALS|_SERVER|_GET|_POST|_COOKIE|_FILES|_ENV|_REQUEST|_SESSION)$/i', $k)) {
...
function securityExit($mesg = 'Unspecified Error')
{
	global $qdig_lang;	
      ...
      exit("<html>\n <head><title>403 {$qdig_lang['Forbidden']}</title></head>
 <body>{$qdig_lang['Forbidden']}: $mesg</body>\n</html>");
} // End securityExit()
...
{
	global $imgs, $qdig_lang;
	$num_imgs = count($imgs);
	$img_num = $image_num + 1;
	if ($num_imgs > 0) {
		$str = "$img_num {$qdig_lang['nav_cntr_txt']} $num_imgs";
		return $str;
	}
} // End imageCounter()
...
function writeData($ind=1) {
	global $header, $get_vars, $reqd_image, $img_sz_labels,
	$pre_gallery, $gdig_version, $qdig_bg_clr_attr, $dir_nav,
	$reqd_img_size_tmp, $ctrl_bar, $dirnav_out, $imgs, 
	$control_bar_on, $ctrl_links_mesg, 
	$no_lnks_view, $omit_navlinks, $namelinks ,
	$namelinks_ena, $thmbs_ena, $sidebar_bg_clr, 
	$sidebar_margin_top, $sidebar_height, $display_area,
	$img_tbl_bg_clr_attr, $subdirs, $empty_gallery_msg,
