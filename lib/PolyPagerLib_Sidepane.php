...
function writeFeedDiv($ind=5)
{
    ...
    if ($feed_amount > 0) {
        ...
        if ($sys_info['hide_public_popups']==0) {
            $text = ' onmouseover="nhpup.popup(\''.__('Below you see a list of the latest entries on this website. This link explains how you can subscribe to them via an RSS-Feed.').'\')" title=""';
        } else {
            $text = '';
        }
        ...
        for ($x=0; $x<count($res); $x++) {
            ...
            if ($sys_info['hide_public_popups']==0) {
                $text = 'onmouseover="nhpup.popup(\''.$tipText.'\')" title="" ';
            } else {
                $text = '';
            }
            global $path_to_root_dir;
