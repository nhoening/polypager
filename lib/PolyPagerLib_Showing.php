...
function getShowParameters()
{
    ...
    if (count(getPageNames()) == 0 and !$sys_info['no_tables'] and !includedByAdminScript()) {
        ...
        if ($params["page"] == "") {
            ...
            if ($_GET["page"]!="") {
                $params["page"] = urldecode($_GET["page"]);
            }
        }
    ...
    if ($params["page"] != "" and isAKnownPage($params["page"])) {
        ...
        //-------------------------cmd param
        $params["cmd"] = $_POST['cmd'];
        //commands: show|_search|Show month|Show year
        if (!isset($params["cmd"])) {
            $params["cmd"] = $_GET['cmd'];
        }
        if (!isset($params["cmd"])) {
        ...
        if (isNumericType($entity['pk_type'])) {
            ...
            if ($params["nr"] == "" and isMultipage($params["page"])) {
                $params["nr"] = $params["max"];
            }
            //no preferation: start with highest entry
        }
        
        //-------------------------step param
        $default_step = $entity["step"];
        //show this much on a page, could be a number or "all"
        if ($default_step == "") {
            $default_step = "all";
        }
        //1. normally one should be shown - but now we show all, briefly
        if ($default_step == "1" and($_POST["nr"] == "" and $_GET["nr"] == "")) {
            $params["step"] = "all";
        }
        //2. a nr is given - show only this entry
        if ($_POST["nr"] != "" or $_GET["nr"] != "") {
            $params["step"] = "1";
        }
        //3. coming in explicitly
        if (isset($_GET["step"])) {
            $params["step"] = $_GET["step"];
        }
        //coming in per GET?
        if (isset($_POST["step"])) {
            $params["step"] = $_POST["step"];
        }
        //coming in per POST?
        ...
        if ($params["group"] == "" and $params["nr"] == "") {
            ...
            if ($glist['valuelist'] != 'standard,') {
                $params["group"] = $page_info["default_group"];
            }
        }
        
        //Search
        if ($params["cmd"] == "_search") {
            //search
            $had_value = false;
            $search = array();
            if ($entity["search"]["range"] == "1") {
            }
            //range has no parameters we haven't covered already
        ...
        if ($debug) {
            echo('                <div class="debug">page param is: '.$params["page"].', topic param is: '.$params["topic"].'</div>'."\n");
        }
    } else {
        // fall back
        ...
        // preview - take params from URLS and pretend they came form the database :-)
            ...
            //we'll select all there is
                ...
                if (($entity["search"]["keyword"] == '1' or $params['page'] == '_search') and $params["search"]["kw"] != "") {
                    ...
                    //lower/upper-case should not matter in our keyword search!
                    if ($said_where) {
                        $a[] = " AND ";
                    } else {
                        $a[] = " WHERE ";
                    ...
                    if ($found_a_textfield) {
                        $a[count($a)-1] = substr_replace($a[count($a)-1],'',-2,2);
                    }
                    //the last OR has to go
                    else {
                        continue;
                    }
                    ...
                    foreach($entity["fields"] as $f) {
                        ...
                        if ($params["search"][$f["name"]] != "" and $f["valuelist"] != "") {
                            ...
                            if ($f["name"] == $entity["group"]["field"]) {
                                $params["group"] = $params["search"][$f["name"]];
                            }
                        }
                    }
...
function writeSearchForm($show, $ind=4)
{
    ...
    global $params;
    global $from_admin;
    //true if the function is called from the admin area - if
    ...
    !(isMultipage($params["page"]) and $page_info["tablename"] == "") ) {
            ...
            if ($params["cmd"] == "show") {
                ...
                if (!($params["nr"] == 0 and $params["max"] == 0)) {
                    ...
                    if ($params["step"] == "1") {
                        ...
                        $prev = $params["nr"] - ($step-1);
                        $step = $entity["step"];
                        //use rather this, not one, to make links
                        $next = $params["nr"] + ($step-1);
                    } else {
                    ...
                    if ($prev > 0) {
                        ...
                        //no link to earlier entries possible
                        echo('                 <i>'.__('previous').'</i>|'."\n");
                    }
                    if ($params["nr"] < $params["max"]) {
                        // link to next entries
                        ...
                        //no link to later entries possible
        ...
        if ($entity["search"]["month"] == "1") {
            ...
            for ($i = 2000; $i <= $datum['year']; $i++) {
                if ($i != $actYear) {
                    ...
                    echo($indent.'            <option selected="selected">'.$i.'</option>'."\n");
                }
            }
            echo($indent.'            </select>'."\n");
            echo($indent.'        </div>'."\n");
        }
        if ($entity["search"]["year"] == "1" and $entity["search"]["month"] == "0") {
            //search for a date
            echo($indent.'        <div class="search_option">'."\n");
        ...
        if ($entity["search"]["keyword"] == "1") {
            ...
            if ($params["search"]["kw"] == "") {
                echo('                                    '.__('for keyword:').' <input class="search_kw" disabled="disabled" type="text" size="12" maxlength="24" name="kw"/>'."\n");
            } else {
                echo('                        '.__('with keyword:').' <input class="search_kw" disabled="disabled" type="text" size="12" maxlength="24" name="kw" value="'.$params['search']['kw'].'"/>'."\n");
            }
            echo($indent.'        </div>'."\n");
        }
        ...
        if ($entity["fields"] != "") foreach($entity["fields"] as $f) {
            if ($f["valuelist"] != "") {
                ...
                foreach($vals as $v) {
                    if ($v[1] != $params["search"][$f["name"]]) {
                        echo('<option value="'.$v[0].'">'.$v[1].'</option>'."\n");
                    } else {
                        echo($indent.'            <option value="'.$v[0].' selected="selected">'.$v[1].'</option>'."\n");
                    }
                }
                echo($indent.'            </select>'."\n");
                echo($indent.'        </div>'."\n");
...
function writeEntry($row, $pagename, $list_view, $ind=5) 
{
    ...
    if ($params["group"] != "") {
        $group_forward = 'group='.urlencode($params["group"]);
    }
    
    if ($params["page"]=="_sys_comments" and $params["cmd"]=="preview") {
        echo($indent.'<div id="comments">');
    }
    ...
    if (!$list_view) {
        if ($page_info["hide_options"] == 0 and !($params['page']=='_search' or $params["cmd"] == "_search" )) {
            ...
            echo($indent.'<div class="show_entry">'."\n");
        }
    } else {
        echo($indent.'<div class="list_entry">'."\n");
    }
    ...
    if (!$list_view) {
        ...
        if ($entity["title_field"] != "") {
            echo($indent.'    <a class="target" name="'.buildValidIDFrom($name).'"></a>'."\n");
        }
    }
    
    $briefly = false;
    //turns true when some fields are not shown
        ...
        foreach($entity["fields"] as $f) {
            ...
            if ($params['page']=='_search' or $params["cmd"] == "_search") {
                if ($f["name"]!=$entity["title_field"]) {
                    if ((isDateType($f['data_type']) and($params["search"]["m"] != "" or $params["search"]["y"] != ""))
                ...
                if ((!$list_view or $entity["title_field"] == $f["name"]) and(!(in_array($f["name"], $hidden_fields)) or $entity["title_field"] == $f["name"] )) {
                    ...
                    if ($f["name"] == $entity["title_field"] and $list_view) {
                        //show some symbols for quick glance
                        echo($indent.'    <div class="adop">'."\n");
                        //option _sys_pages can be two things.
                        if ($pagename == "_sys_pages") {
                            if (isSinglepage($unescaped_content)) {
                                $page = "_sys_singlepages";
                            } else if (isMultipage($unescaped_content)) {
                                $page = "_sys_multipages";
                            } else {
                                $page = "_sys_singlepages";
                            }
                            //we shouldn't come here, well, take the more probable
                        } else {
                            $page = $pagename;
                        }
                        $q = getEditQuery('delete', $row[$entity['pk']]);
                        // queries for URL (only happens in Admin area, not public!)
                    ...
                    if ($params["page"] == '_sys_comments' and $f['name'] != 'comment') {
                        ...
                        if (strstr($f['embed_in'], '[CONTENT]') and $f['embed_in'] != "") {
                            $content = str_replace('[CONTENT]', $content, $f['embed_in']);
                        }
                        echo($content);
    ...
    if (!$list_view and $something_was_not_brief == true and $params["step"] != 1) {
        //show a link to the whole entry
    ...
    if (!$list_view and $params["page"]!="_search") {
        showRelatedEntries($row[$entity['pk']], ++$ind);
    }
