...
function getEditParameters()
{
    ...
    if ($params["page"] == "") {
        ...
        if (utf8_strpos($query_array[0], "page=") !== false) {
    ...
    if ($params["page"] != "" and isAKnownPage($params["page"])) {
        ...
        if ($params["group"] == '') {
            $params["group"] = urldecode($_POST["group"]);
        }
        //coming in per POST?
        ...
        if ($params["feed"] == "") {
            $params["feed"] = $_GET["_formfield_feedbox"];
        }
        if ($params["feed"] == "on") {
            $params["feed"] = "1";
        } else {
        ...
        if ($params["cmd"] != "") {
            ...
            if ($params["cmd"] == "show" or $params["cmd"] == "entry" or $params["cmd"] == "edit" or $params["cmd"] == "delete"  or $params["cmd"] == "preview") {
                //get data
                $consistency_fields = utf8_explode(",",$entity["consistency_fields"]);
                $values = array();
                foreach($entity["fields"] as $f) {
                    if ($f['valuelist'] != "") {
                        //try possible new entry first
                        $values[$f["name"]] = $_POST['_formfield_'.$f["name"].'_new'];
                        if (!isset($values[$f["name"]]) or $values[$f["name"]]=="") {
                            $values[$f["name"]] = $_GET['_formfield_'.$f["name"].'_new'];
                        }
                    }
                    if (!isset($values[$f["name"]])) {
                        $values[$f["name"]] = $_POST['_formfield_'.$f["name"]];
                    }
                    if (!isset($values[$f["name"]])) {
                        $values[$f["name"]] = $_GET['_formfield_'.$f["name"]];
                    }
                    ...
                    if (in_array($f["name"], $consistency_fields)) {
                        ...
                        if (!isset($values['old_formfield_'.$f["name"]])) {
                            $values['old_formfield_'.$f["name"]] = $_GET['old_formfield_'.$f["name"]];
                        }
                    }
                }
            ...
            if ($params["values"]["nogarbageplease_"] == "") {
                $params["values"]["nogarbageplease_"] = $_GET["nogarbageplease_"];
            }
            ...
            and $entity["pk"] != "") {
                ...
                if ($params["nr"] == "" and $params["cmd"] == "show") {
                    $params["cmd"] = "entry";
                }
                //assume new one
            }
...
function getEditQuery($command, $theID)
{
    ...
    if ($theID == "") {
        $theID = $params["values"][$entity["pk"]];
    }
    $query = "";
    # we'll build in this string
    $theParams = array();
    #... add params here
    $queries = array();
    # ... and add it to this array we'll return
    ...
    if ($params['cmd'] == "edit" || $params['cmd'] == "entry" || $params['cmd'] == "delete") {
        ...
        foreach($can as $c) {
            if ($c[1] <= 2) {
                ...
                if ($command != "entry") {
                    $queries[] = "DELETE FROM `".$c[0]."` WHERE ".nameEqValueEscaped($fk_field['data_type'], $c[2][0]['fk']['field'], $fk_val);
                }
...
function handleFeed()
{
    ...
    $p = 1;
    if ($entity["publish_field"]!="" and $params["values"][$entity["publish_field"]] == '0') {
        $p = 0;
    }
    ...
    if ($params["feed"] == '1') {
        ...
        if ($params["cmd"] != "delete") {
            ...
            if ($entity['title_field'] != "") {
                $title = $params['values'][$entity['title_field']];
            }
            $tfield = guessTextField($entity);
            if ($title == "" and $tfield != "") {
                $title = getFirstWords($params['values'][$tfield],50);
            }
