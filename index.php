...
if ($sys_info['whole_site_admin_access']) {
    require_once('admin'.FILE_SEPARATOR.'auth.php');
}
...
$area = "";
//one of '', '_admin', '_gallery' - makes your template flexible if you want
...
if (!$known_page or($show_params["page"] != '_sys_comments' and isASysPage($show_params["page"]))) {
    ...
    if ($page_info['only_admin_access']) {
        require_once('admin'.FILE_SEPARATOR.'auth.php');
    }
    ...
    $i_manipulated = true;
    //positive assumption
    if ($show_params["cmd"] == "entry") {
        // we have a comment entry (for this script there can only be those to be edited)
        ...
        if ($msg == "") {
            ...
            if ($fehler_nr != 0) {
                ...
                if ($debug) {
                    echo('<div class="debug">I used this query: '.$query.'.</div>'."\n");
                }
                $params["cmd"]="show";
            }
