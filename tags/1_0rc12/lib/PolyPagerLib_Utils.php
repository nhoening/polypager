...
function isMultipage($page_name)
{
    ...
    $mp =  getMultipages();
    foreach($mp as $p)  if ($p["name"] == $page_name) {
        return true;
    }
    return false;
}
...
function getSysInfo()
{
    ...
    if ($fehler_nr!==0) {
        ...
        if ($_GET['cmd']!='create' and eregi("doesn't exist", $fehler_text) and $params["cmd"] != 'create') {
            $sys_info = array();
            $sys_info['no_tables'] = true;
            // remember that we didn't see the table
        } else {
...
function getTables()
{
    ...
    if ($non_sys_tables == "") {
        ...
        while ($row = mysqli_fetch_array($tables)) {
            $table_name = $row[0];
            if (utf8_substr($table_name,0,5) != "_sys_") {
                $non_sys_tables[] = $table_name;
            }
        }
    }
        ...
        if ($page_info == "" or $page_info["name"] != $page_name) {
            if (getAlreadyBuiltPageInfo($page_name) != "") {
                ...
                if (isSinglePage($page_name)) {
                    ...
                    //no page? maybe a table. try the first page for this table
                ...
                } else if (isASysPage($page_name)) {
                    $page_info["tablename"] = $page_name;
                }
                //adding this for comment preview
                if ($page_name=='_sys_comments') {
                    $page_info["hide_toc"] = 1;
                }
            }
        }
    }
