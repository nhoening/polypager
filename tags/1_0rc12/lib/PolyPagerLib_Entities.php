...
*/
$entity = "";
//stores the actual entity
$old_entities = "";
//stores entites we already built within this request
    ...
    if ($entity == "" or $entity["pagename"] != $page_name) {
        ...
        getAlreadyBuiltEntity($page_name) != "") {
            ...
            if ($page_name == "_sys_sys") {
                $entity["tablename"] = "_sys_sys";
                //there is no _sys_sys - table
                $entity["one_entry_only"] = "1";
                //keep it one
                
                $entity = addFields($entity,$entity["tablename"]);
                $skindirs = scandir_n('../style/skins', 0, false, true);
                $skindirs_wo_picswap = array();
                for ($x=0; $x<count($skindirs); $x++) {
                    //picswap gets extra handling in PolyPagerLib_HTMLFraming
                    if ($skindirs[$x] != 'picswap') {
                        $skindirs_wo_picswap[] = $skindirs[$x];
                    }
                }
...
//	table for fields
    ...
    if ($_GET["cmd"] != 'new' and $_POST["cmd"] != 'new') {
        $entity["disabled_fields"] .= ',name';
    } else {
        $param_group = urldecode($_GET["group"]);
        if ($param_group == '') {
            $param_group = urldecode($_POST["group"]);
        }
    ...
    setEntityFieldValue("validation", "valuelist", 'no validation,number,any_text,email');
    ...
    if ($page_info["group_field"] != "" and $page_info["group_field"] == $f['name']) {
        ...
        global $params;
        if ($params['cmd'] == "new" and $params["values"] = "") {
            $params['values'] = array();
        }
    ...
    $entity["hidden_form_fields"] = "id,foreign_key_to,on_update,on_delete";
    //this feature is not clearly defined as from 0.9.7
...
//	table for comments
    ...
    $entity["hidden_fields"] = "id,pagename,pageid,is_spam,email";
...
//single pages
    ...
    setEntityFieldValue("the_group", "valuelist_from_db", true);
    //user cannot add any via entry form
    ...
    setEntityFieldValue("the_group", "help", __('the group of this entry (you will find the groups in the specifications for this page). The standard group contains entries that are always shown.'));
            ...
            if ($entity["title_field"] == "") {
                $entity["title_field"] = guessTextField($entity, false);
            }
            ...
            if (isMultipage($page_name) || isSinglepage($page_name)) {
                ...
                foreach($ref_tables as $rt) {
                    ...
                    if ($rt['table_name'] != "") {
                        ...
                        if ($rt["title_field"] != "") {
                            $q .= ", ".$rt['title_field']." as tf ";
                        }
                        $q .= " FROM `".$rt['table_name'].'`';
                        //singlepages can operate on the page level whith all data being in one table...
                        if ($rt['fk']['ref_page'] != '' and isSinglepage($rt['fk']['ref_page'])) {
                            $q .= " WHERE pagename = '".$rt['fk']['ref_page']."'";
                        }
                        if ($rt["title_field"] != "") {
                            $q .= " ORDER BY tf";
                        }
                        ...
                        foreach($result as $row){
                            if (!in_array($row['pk'], $used_ids)) {
                                $tmp[] = '|'.$row['pk'].':'.$row['tf'].'|';
                                //protect possible values containing commas with | (internal convention for valuelists)
                                $used_ids[] = $row['pk'];
                            }
                        }
...
{
    if ($entity["fields"] != "") {
        foreach($entity["fields"] as $f) if ($f["name"] == $fname) return $f;
    }
    return "";
}
